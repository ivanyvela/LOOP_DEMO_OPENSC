...
ANIMATION_CONFIG = {
    ...
    # 'Nitrate' uses the NO3- line. If not found, falls back to constant vertical.
    'MASTER_SPEED_TRACK': 'Nitrate', 
    
    # Per-borehole animation configuration.
    # By default, all boreholes are animated, full depth interval is used, and no logs are excluded.
    # To adjust, find the ID below and modify its parameters:
    #   'animate': True or False (whether to show the moving dots for this borehole)
    #   'interval': 'all' or [start_depth, end_depth] (e.g., [2, 5] to only animate between 2m and 5m)
    #   'exclude_logs': ['Apr-20 a Pt', 'Apr-20 b Pt'] (list of string matches to hide specific redox logs from animation)
     'BOREHOLE_OPTIONS': {
...
def get_param_color(param_name):
    if param_name in MANUAL_COLORS:
        return MANUAL_COLORS[param_name]
    if param_name not in assigned_colors:
        assigned_colors[param_name] = next(color_gen)
...
for loop_nr, display_id, normalized_id in valid_plots:
    ...
    fig.suptitle(
        f"{loop_nr} - Borehole {display_id} - DGUnr {dgu_nr}",
    ...
    plot_geochem_track(plt.subplot(gs[6]), sub_geo, [no3_col,'SO₄²⁻ [mg/L]','Cl⁻ [mg/L]'], [], "Anions 1\n[mg/L]",
    ...
    if bh_config.get('animate', True):
        ...
        if ANIMATION_CONFIG.get('MASTER_SPEED_TRACK', '').lower() == 'nitrate' and nitrate_lines_info:
    ...
    save_name = "" + f"{loop_nr}_{display_id}.png"
    save_path = os.path.join(OUTPUT_PLOTS_DIR, save_name)
    fig.savefig(save_path, dpi=300)
    plt.close(fig)
    all_animations["" + f"{loop_nr}_{display_id}"] = anim_data
